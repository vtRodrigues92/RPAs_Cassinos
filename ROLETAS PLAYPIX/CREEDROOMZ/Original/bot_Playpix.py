...
#BANCO DE DADOS
#def alimenta_banco_dados(nome_cassino, historico_roleta, dicionario_roletas, estrategia, validador):
#    global lista_anterior
    #'''banco de dados
    #IP: sql818.main-hosting.eu
    #User: u791277084_userroletapp
    #Pass: ShOiPvwhG9q@
    #DB: u791277084_dbroletapp
    #Tabela: historico '''

   
    #if lista_anterior == [] or historico_roleta != dicionario_roletas[nome_cassino] or dicionario_roletas == {}:

    #    resultado_atual = dicionario_roletas[nome_cassino][0]

    #    try:
    #        ''' CONECTANDO COM O BANCO '''
    #        db_conexao = mysql.connector.connect(host='sql818.main-hosting.eu',
    #                                             database='u791277084_dbroletapp',
    #                                             user='u791277084_userroletapp',
    #                                             password='ShOiPvwhG9q@')

            #Variavel que executa as querys
    #        cursor = db_conexao.cursor()

    #        ''' QUERY '''
    #        query_inserir_dados = (f"""INSERT INTO u791277084_dbroletapp.historico 
    #                                                                VALUES(NULL,
    #                                                                      '{datetime.now()}',
    #                                                                      'CREEDROOMZ',
    #                                                                      '{nome_cassino}',
    ...
    #        lista_anterior = dicionario_roletas[nome_cassino][0]
...
def resgatar_historico(roleta):
    ...
    try:
        ...
        historico_resultados = browser.find_elements_by_css_selector('.lobby .lobby-roulette-stats')
        lista_resultados = []
...
def formatar_resultados(roleta):  
    ...
    try:
        ...
        for numero in resultados:
            if 'New' in numero or 'Hot' in numero:
                continue
...
def logarSite():
    ...
    try:
        ...
        time.sleep(3)
        browser.find_element_by_xpath('//*[@id="root"]/div[6]/div/div/div/div/div/div[2]/form/div[1]/div[6]/div').click()   #Clicando no btn login
        time.sleep(20)
    except:
        pass
    ...
    try:
        for canal in canais:
            try:
                ...
                #Variavel Dinâmica
                globals()[f'alerta_{canal}'] = bot.send_message(canal, 
    ...
    try:
        for canal in canais:
            try:
                ...
                globals()[f'sinal_{canal}'] = bot.send_message(canal, 
...
def checkSinalEnviado(lista_proximo_resultados, nome_cassino, roleta):
    ...
    while contador_cash <= 2:
        ...
        try:
            ...
            lista_resultados_sinal = formatar_resultados(roleta)
                    ...
                    try:
                        ...
                        for canal in canais:
                            try:
                                ...
                                bot.reply_to(globals()[f'sinal_{canal}'], message_green.replace('[RESULTADO]', ' | '.join(resultados)),
                                                        parse_mode='HTML')
                            except:
                                pass
        ...
        try:
            ...
            for canal in canais:
                try:
                    ...
                    bot.reply_to(globals()[f'sinal_{canal}'], message_red.replace('[RESULTADOS]', ' | '.join(resultados)),
                                        parse_mode = 'HTML')
                except:
                    pass
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot de Roletas Playpix Iniciado! ✅ Escolha uma opção 👇",
                                    reply_markup=markup)
...
def registrarTipoEstrategia(message_tipo_estrategia):
    ...
    if message_tipo_estrategia.text in ['ESTRATÉGIAS PADRÕES']:
        ...
        estrategias_padroes = (['repetição', '1ª coluna', '5', '2ª/3ª coluna'],
                               ['repetição', '2ª coluna', '5', '1ª/3ª coluna'], 
                               ['repetição', '3ª coluna', '5', '1ª/2ª coluna'], 
                               ['repetição', '1ª duzia', '5', '2ª/3ª duzia'], 
                               ['repetição', '2ª duzia', '5', '1ª/3ª duzia'], 
                               ['repetição', '3ª duzia', '5', '1ª/2ª duzia'],
                               ['repetição', 'Números par(es)', '5', 'Números impar(es)'],
                               ['repetição', 'Números impar(es)', '5', 'Números par(es)'],
                               ['repetição', 'Números baixos', '5', 'Números altos'],
                               ['repetição', 'Números altos', '5', 'Números baixos'])
