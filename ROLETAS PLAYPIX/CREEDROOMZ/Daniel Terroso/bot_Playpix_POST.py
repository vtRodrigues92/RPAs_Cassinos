    ...
    def run(self):
        try:
            ...
            payload = {
                    ...
                    'link_game_bet':'https://games.playpix.com/LaunchGame'
            }
    ...
    try:
        ...
        for estrategia in lista_estrategias:
            ...
            if validador.count('true') == int(sequencia_minima)-1:
                ...
                print('ENVIAR ALERTA')
                enviar_alerta_telegram(dicionario_roletas, nome_cassino, sequencia_minima, estrategia)
                time.sleep(1)
                        ...
                        if dicionario_roletas[nome_cassino][:3] != lista_proximo_resultados[:3]:
                            ...
                            if estrategia[0] == 'repetição':
                                ...
                                if lista_proximo_resultados[0] in aposta_externa[estrategia[1]]:
                                    ...
                                    enviar_sinal_telegram(nome_cassino, sequencia_minima, estrategia)
                                    print('=' * 220)
...
def enviar_alerta_telegram(dicionario_roletas, nome_cassino, sequencia_minima, estrategia):
    global contador_passagem
    ...
    try:
        ...
        texto = message_alerta.replace('[NOME_CASSINO]',nome_cassino.title())\
    ...
    try:
        for key, value in canais.items():
            try:
                ...
                table_alerta = message_alerta.replace('[NOME_CASSINO]',nome_cassino.title())\
                                            .replace('[ESTRATEGIA]',estrategia[0].title()+' de '+estrategia[1].title())\
                                            .replace('[LISTA_RESULTADOS]', ' | '.join(dicionario_roletas[nome_cassino][:int(sequencia_minima)-1]))\
...
def enviar_sinal_telegram(nome_cassino, sequencia_minima, estrategia):
    global table_sinal
    ...
    with open('arquivos_txt\\sinal.txt',"r", encoding="utf-8") as arquivo:
        mensagem_sinal = arquivo.read()


    #ENVIANDO POST PARA A API
    try:
    
        texto = mensagem_sinal.replace('[APOSTA]', estrategia[3].upper())\
    ...
    try:
        for key, value in canais.items():
            try:
                ...
                table_sinal = mensagem_sinal.replace('[APOSTA]', estrategia[3].upper())\
                                            .replace('[NOME_CASSINO]', nome_cassino.title())\
                                            .replace('[ESTRATEGIA]', estrategia[0].title()+' de '+estrategia[1].title())\
                                            .replace('[LISTA_RESULTADOS]', ' | '.join(dicionario_roletas[nome_cassino][:1]))\
                                            .replace('[LINK_CADASTRO]', value[0])
                ...
                if lista_resultados_sinal[0] in grupo_apostar[estrategia[3]] or lista_resultados_sinal[0] == '0' or lista_resultados_sinal[0] == '00':
                    ...
                    try:
                        ...
                        try:
                            ...
                            texto = message_green.replace('[RESULTADO]', ' | '.join(resultados))
                        ...
                        for key,value in canais.items():
                            try:
                                ...
                                responderMensagemTelegram(key, message_green.replace('[RESULTADO]', ' | '.join(resultados))).start()
        ...
        try:
            ...
            for key, value in canais.items():
                try:
                    ...
                    responderMensagemTelegram(key, message_red.replace('[LISTA_RESULTADOS]', ' | '.join(resultados))).start()
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot Roleta A (PLAYPIX) Iniciado! ✅ Escolha uma opção 👇",
                                    reply_markup=markup)
