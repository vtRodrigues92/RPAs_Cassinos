...
print()
print('                                #################################################################')
print('                                #################     ROLETAS PLAYPIX     #######################')
print('                                #################     SEMI AUTOMÁTICO     #######################')
print('                                #################################################################')
...
def coletarResultados(lista_roletas):
    ...
    while True:
        ...
        try:
            ...
            for roleta in enumerate(roletas_com_historico):
                ...
                while True:
                    try:
                        ...
                        if dicionario_roletas[nome_cassino][:3] != lista_proximo_resultados[:3]:
                            ...
                            ### ALIMENTANDO BANCO DE DADOS ###
                            #alimenta_banco_dados(nome_cassino,lista_proximo_resultados, dicionario_roletas, 'NULL','NULL')
...
def cadastrarEstrategia(message):
    ...
    if contador_passagem == 0:
        ...
        markup_tipo = markup_tipo.add('◀ Voltar', 'REPETIÇÃO', 'AUSÊNCIA', 'ANÁLISE SEMI AUTOMÁTICA')    
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot Roletas Playpix Semi-Automático Iniciado! ✅ Escolha uma opção 👇",
                                    reply_markup=markup)
...
def registrarTipoEstrategia(message_tipo_estrategia):
    ...
    if message_tipo_estrategia.text in ['ANÁLISE SEMI AUTOMÁTICA']:
        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True, row_width=2)
        markup = markup.add('/start','✅ Ativar Bot','⚙🧠 Cadastrar Estratégia', '⚙🎰 Cadastrar Roletas', '🧠📜 Estratégias Cadastradas', '🎰📜 Roletas Cadastradas', '🗑🧠 Apagar Estratégia', '🗑🎰 Apagar Roleta', '⏲ Ultimos Resultados', '📊 Placar Atual','📈 Gestão','🛑 Pausar Bot')
        ...
        estrategias_padroes = (['repetição', '1ª coluna', '5', '2ª/3ª coluna'],
                               ...
                               ['repetição', 'Cor vermelho', '5', 'Cor preto'],
                               ['repetição', 'Cor preto', '5', 'Cor vermelho'])
        ...
        message_aposta = bot.reply_to(message_tipo_estrategia, "🤖 Bot Semi-Automático Configurado ✅✅✅", reply_markup=markup)
