                    ...
                    try:
                        ...
                        try:
                            ...
                            if 'Esports' in browser.find_element_by_xpath('//*[@class="lsb-ScoreBasedScoreboardAggregate "]').text or\
                              ...
                              'Femenino' in browser.find_element_by_xpath('//*[@class="lsb-ScoreBasedScoreboardAggregate "]').text or\
                              'Feminino' in browser.find_element_by_xpath('//*[@class="lsb-ScoreBasedScoreboardAggregate "]').text: 
                                continue
                        ...
                        try:
                            ...
                            try:
                                ...
                                browser.find_element_by_xpath('//*[@class="lv-ButtonBar_MatchLiveIcon me-MediaButtonLoader me-MediaButtonLoader_ML1 "]').click()
                        ...
                        ###VALIDANDO GREEN DOS SINAIS ENVIADOS
                        check_green()
                        ...
                        if ataq_perig_time1 > ataq_perig_time2:
                            ...
                            validador_time1(ataque_perigoso, finalizacoes, escanteio, tempo_jogo, placar_referencia)
...
def validador_time1(ataque_perigoso, finalizacoes, escanteio, tempo_jogo, placar_referencia):
    global lista_sinais_enviados, time_referencia
    ...
    try:
        ...
        if appm >= usuario_appm and cg >= usuario_cg_ht and\
           ...
           int(placar_time1) == 0 and int(placar_time2) == 0 and placar_jogo[0]+' x '+placar_jogo[3]+' x '+'HT' + time1 not in lista_sinais_enviados:
            
            enviarSinalTelegram(cg, appm, time_referencia, 'HT')
        ...
        if appm >= usuario_appm and cg > usuario_cg_ft and\
           ...
           int(placar_time1) == 0 and int(placar_time2) == 0 and  placar_jogo[0]+' x '+placar_jogo[3]+' x '+'FT' + time1 not in lista_sinais_enviados:

            enviarSinalTelegram(cg, appm, time_referencia, 'FT')
            #thread = CheckSinalThread()
            #thread.start()
...
def validador_time2(ataque_perigoso, finalizacoes, escanteio, tempo_jogo, placar_referencia):
    ...
    try:
        ...
        if appm >= usuario_appm and cg >= usuario_cg_ht and\
           ...
           int(placar_time1) == 0 and int(placar_time2) == 0 and\
           placar_jogo[0]+' x '+placar_jogo[3]+' x '+'HT' + time2 not in lista_sinais_enviados:
        ...
        if appm >= usuario_appm and cg > usuario_cg_ft and\
           ...
           int(placar_time1) == 0 and int(placar_time2) == 0 and\
           placar_jogo[0]+' x '+placar_jogo[3]+' x '+'FT' + time2 not in lista_sinais_enviados:
...
def enviarSinalTelegram(cg, appm, time_referencia, tempo_abrev):
    global table_sinal, indice_sinal
    ...
    try:
        ...
        if aposta_escanteio != None and aposta_gol != None:
            ...
            msg_sinal = open("sinal.txt", "r", encoding="utf-8").read()

            # Add Jogo na Lista de Sinais e pegando o indice
            lista_sinais_enviados.append(placar_jogo[0]+' x '+ placar_jogo[3] +' x ' + tempo_abrev + time_referencia)
            indice_sinal = lista_sinais_enviados.index(f'{placar_jogo[0]} x {placar_jogo[3]} x {tempo_abrev}{time_referencia}')
            ...
            try:
                for key, value in canais.items():
                    try:
                        ''' Mensagem '''
                        table_sinal = msg_sinal\
                                      .replace('[APPM_TIME]', str(appm))\
                                      .replace('[CG_TIME]', str(cg))\
                                      .replace('[LINK]', url_jogo)\
                                      .replace('[TIME1]', time1).replace('[TIME2]', time2)\
                                      .replace('[TIME]', time_referencia)


                        ''' ENVIANDO SINAL TELEGRAM PARA O CANAL NO LOOP FOR CORRENTE '''
                        globals()[f'sinal_{key}_{indice_sinal}'] = bot.send_message(key, table_sinal, parse_mode='HTML', disable_web_page_preview=True)
...
def check_red(sinal):
    ...
    ## RESPONDER COM RED
        try:
            ...
            for key, value in canais.items():
                try:
                    bot.reply_to(globals()[f'sinal_{key}_{sinal}'], mensagem_green[17].replace('\n',''), parse_mode = 'HTML')
                except:
                    pass
            
            del lista_sinais_enviados[sinal]
...
def check_green():
    ...
    #dif_placar = int(placar_time1) - int(placar_time2)

    for sinal_enviado in lista_sinais_enviados:

        ###GREEN TIME 1 PRIMEIRO TEMPO
        if sinal_enviado == placar_jogo[0]+' x '+placar_jogo[3]+' x '+'HT'+ time1:

            index = lista_sinais_enviados.index(f"{placar_jogo[0]} x {placar_jogo[3]} x HT{time1}")
            
            ### VALIDAR SE FOI GREEN MULTIPLA OU OVER 0.5
            if int(placar_time1) != 0 and int(tempo_jogo[:1]) <= 46:
                
                ###RESPONDE MSG DE GREEN MULTIPLA
                msg_green('MULTIPLA', 'PRIMEIRO TEMPO', 'HT', time1, index)

            elif int(placar_time2) != 0 and int(tempo_jogo[:1]) <= 46:

                ###RESPONDE MSG DE GREEN OVER
                msg_green('OVER 0.5', 'PRIMEIRO TEMPO', 'HT', time1, index)

            elif int(placar_time1) != 0 and int(tempo_jogo[:1]) > 46:

                ###RESPONDE MSG DE GREEN OVER
                msg_green('OVER 0.5', 'SEGUNDO TEMPO', 'HT', time1, index)
        ...
        ###GREEN TIME 2 PRIMEIRO TEMPO
        elif sinal_enviado == placar_jogo[0]+' x '+placar_jogo[3]+' x '+'HT'+time2:
            
            index = lista_sinais_enviados.index(f"{placar_jogo[0]} x {placar_jogo[3]} x HT{time2}")

            ### VALIDAR SE FOI GREEN MULTIPLA OU OVER 0.5
            if int(placar_time2) != 0 and int(tempo_jogo[:1]) <= 46:
        ...
        ###GREEN TIME 1 SEGUNDO TEMPO
        elif sinal_enviado == placar_jogo[0]+' x '+placar_jogo[3]+' x '+'FT'+time1:

            index = lista_sinais_enviados.index(f"{placar_jogo[0]} x {placar_jogo[3]} x FT{time1}")

            ### VALIDAR SE FOI GREEN MULTIPLA OU OVER 0.5
            if int(placar_time1) != 0:
                
                ###RESPONDE MSG DE GREEN MULTIPLA
                msg_green('MULTIPLA', 'SEGUNDO TEMPO', 'FT', time1, index)

            elif int(placar_time2) != 0:

                ###RESPONDE MSG DE GREEN MULTIPLA
                msg_green('OVER 0.5', 'SEGUNDO TEMPO', 'FT', time1, index)
                

        
        ###GREEN TIME 2 SEGUNDO TEMPO
        elif sinal_enviado == placar_jogo[0]+' x '+placar_jogo[3]+' x '+'FT'+time2:

            index = lista_sinais_enviados.index(f"{placar_jogo[0]} x {placar_jogo[3]} x FT{time2}")

            ### VALIDAR SE FOI GREEN MULTIPLA OU OVER 0.5
            if int(placar_time2) != 0:
                
                ###RESPONDE MSG DE GREEN MULTIPLA
                msg_green('MULTIPLA', 'SEGUNDO TEMPO', 'FT', time1, index)

            elif int(placar_time1) != 0:
...
def msg_green(aposta, tempo, tempo_abrev, time_referencia, indice_sinal):
    global lista_sinais_enviados
    ...
    try:
        ...
        ''' Lendo o arquivo txt config-mensagens '''
        msg_green = open("green.txt", "r", encoding="utf-8").read()
        
        for key, value in canais.items():
            try:
                bot.reply_to(globals()[f'sinal_{key}_{indice_sinal}'], msg_green.replace('[APOSTA]', aposta).replace('[TEMPO]', tempo), parse_mode='HTML')
            except:
                pass
        
        ####REMOVENDO DA LISTA DE SINAIS ENVIADOS
        lista_sinais_enviados.pop(indice_sinal)
