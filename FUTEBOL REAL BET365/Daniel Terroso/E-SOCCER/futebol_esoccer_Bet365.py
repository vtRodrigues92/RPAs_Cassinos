    ...
    while True:
        ...
        while True:
            try:
                ...
                competicoes = browser.find_elements_by_xpath('//*[@class="ovm-CompetitionHeader_NameText "]')
                ...
                for competicao in competicoes:
                    ...
                    if 'E-soccer' in competicao.text and '12' in competicao.text\
                        or 'Esoccer' in competicao.text and '12' in competicao.text\
                        or 'eSports' in competicao.text and '12' in competicao.text:
                        competicao.click()
                        pass
...
def validador_time1(ataque_perigoso, finalizacoes, escanteio, tempo_jogo, placar_referencia):
    ...
    try:
        ...
        if appm >= usuario_appm and cg >= usuario_cg_ht and cg_time2 >= usuario_cg_ht and\
           int(tempo_jogo[:-3]) >= usuario_tempo_inicial_ht and\
           int(placar_time1) == 0 and int(placar_time2) == 0 and\
           int(tempo_jogo[:-3]) <= usuario_tempo_final_ht and placar_jogo[0]+' x '+placar_jogo[3]+' x '+'HT' not in lista_sinais_enviados:
        ...
        if appm >= usuario_appm and cg > usuario_cg_ft and cg_time2 >= usuario_cg_ft and\
           int(tempo_jogo[:-3]) >= usuario_tempo_inicial_ft and\
           int(placar_time1) == 0 and int(placar_time2) == 0 and\
           int(tempo_jogo[:-3]) < usuario_tempo_final_ft and placar_jogo[0]+' x '+placar_jogo[3]+' x '+'FT' not in lista_sinais_enviados:
...
def validador_time2(ataque_perigoso, finalizacoes, escanteio, tempo_jogo, placar_referencia):
    ...
    try:
        ...
        # VALIDAÇÃO DO FUNIL HT ( PRIMEIRO TEMPO )
        if appm >= usuario_appm and cg >= usuario_cg_ht and cg_time2 >= usuario_cg_ht and\
           int(tempo_jogo[:-3]) >= usuario_tempo_inicial_ht and int(tempo_jogo[:-3]) <= usuario_tempo_final_ht\
           and int(placar_time1) == 0 and int(placar_time2) == 0\
            and placar_jogo[0]+' x '+placar_jogo[3]+' x '+'HT' not in lista_sinais_enviados:
        ...
        # VALIDAÇÃO DO FUNIL FT ( SEGUNDO TEMPO )
        if appm >= usuario_appm and cg > usuario_cg_ft and cg_time2 >= usuario_cg_ft and\
           int(tempo_jogo[:-3]) > usuario_tempo_inicial_ft and int(tempo_jogo[:-3]) < usuario_tempo_final_ft\
           and int(placar_time1) == 0 and int(placar_time2) == 0\
            and placar_jogo[0]+' x '+placar_jogo[3]+' x '+'FT' not in lista_sinais_enviados:
...
def pegar_link_jogo(time1, time2):
    ...
    try:
        ...
        return url_jogo, aposta_gol

    except:
        pass

    
def enviarSinalTelegram(cg, appm):
    global table_sinal

    try:
        
        # PEGANDO O LINK DO JOGO
        url_jogo, aposta_gol = pegar_link_jogo(time1, time2)
...
def valida_sinal_enviado():
    try:

        if placar_jogo[0]+' x '+placar_jogo[3]+' x '+'HT' in lista_sinais_enviados and int(tempo_jogo[:-3]) <= usuario_tempo_final_ht:

            if int(placar_time1) != 0 or int(placar_time2) != 0:

                ####RESPONDENDO COM GREEN
                ''' Lendo o arquivo txt canais '''
                txt = open("canais.txt", "r", encoding="utf-8")
                arquivo = txt.readlines()
                canais = arquivo[7].replace('canais= ','').replace('\n','')
                ...
                for key, value in canais.items():
                    ...
                    bot.reply_to(globals()[f'sinal_{key}'], "GREEN ✅✅", parse_mode='HTML', disable_web_page_preview=True)

                ####REMOVENDO DA LISTA DE SINAIS ENVIADOS
                lista_sinais_enviados.remove(placar_jogo[0]+' x '+placar_jogo[3]+' x '+'HT')
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot E-Soccer Bet 365 Ativado! ✅ Escolha uma opção 👇",
                                    reply_markup=markup)
