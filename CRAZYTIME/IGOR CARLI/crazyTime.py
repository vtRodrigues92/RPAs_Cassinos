...
def validarJogoPausado():
    try:
        ...
        for sessao in sessao_expirada:
            if sessao.text == 'SESSÃO EXPIRADA' or sessao.text == 'MESA FECHADA':
                logarSite()


        sessao_desconectada = browser.find_elements_by_xpath('//*[@class="contentElement--e8ecb"]')
        for sessao_desc in sessao_desconectada:
            if sessao_desc.text == 'Você foi desconectado. Feche esta janela e conecte novamente para jogar.':
                logarSite()

        

        att_pagina = browser.find_elements_by_css_selector('.contentElement--e8ecb')
...
# FUNÇÃO QUE ARRUMA O HISTÓRICO DE REULTADOS
...
# FUNÇÃO QUE PEGA O HISTORICO DE RESULTADOS DA ROLETA
def coletar_dados():
    while True:
        ...
        try:
            ...
            # Chama a Função que Arruma os Resultados e Insere em uma Lista
            ...
            #VALIDANDO SE SAIU BONUS E ENVIAR SINAL TELEGRAM
            if lista_resultados[-1] == 'bonus':
                estrategia_bonus = ['bonus','bonus','5']
                print('ESTRATEGIA DE BONUS!! --- SINAL TELEGRAM!!')
                enviar_sinal_telegram(estrategia_bonus)
                checar_sinal_enviado(lista_resultados, estrategia_bonus) 
...
def validar_estrategias(lista_resultados):
    ...
    # Percorrendo Estratégias a Fim de Encontrar Padrões
    for estrategia in estrategias:
        ...
        # AUSÊNCIA
        if estrategia[0] == 'ausência': #and estrategia[1:sequencia_minima_alerta] not in lista_resultados[-sequencia_minima_alerta:]:
            # VALIDAR SE A SEQ DA ESTRATÉGIA NÃO ESTÁ NA LISTA DE HISTORICOS DE RESULTADOS
            validador = []
            for n in estrategia[-sequencia_minima_alerta:]:
                if lista_resultados[-sequencia_minima_alerta:].count(n) == 0:
                    validador.append('true')
            
            if validador.count('true') == sequencia_minima_alerta:
                ...
                while True:
                    ...
                    try:
                        ...
                        # Função que Arruma os Dados Coletados
                        lista_resultados_validacao = gerarListaResultados(historico_resultados)

                        # Valida se a lista de resultados atual é a mesma da lista definida antes de enviar o Sinal
                        if lista_resultados != lista_resultados_validacao:
                            
                            validador = []
                            for n in estrategia[-sequencia_minima_sinal:]:
                                if lista_resultados[-sequencia_minima_sinal:].count(n) == 0:
                                    validador.append('true')

                            # Verificando se o ultimo resultado da mesa está dentro da estratégia
                            if validador.count('true') == sequencia_minima_sinal:
        ...
        # REPETIÇÃO
        if estrategia[0] == 'repetição' and estrategia[1:sequencia_minima_alerta] == lista_resultados[-sequencia_minima_alerta:]:
            print('IDENTIFICADO O PADRÃO DA ESTRATÉGIA --> ', estrategia)
            print('ENVIAR ALERTA')
            enviar_alerta_telegram()
            time.sleep(1)
            ...
            while True:
                ...
                try:
                    ...
                    if lista_resultados != lista_resultados_validacao:
                        # Verificando se o ultimo resultado da mesa está dentro da estratégia
                        if estrategia[1:sequencia_minima_sinal] == lista_resultados_validacao[-sequencia_minima_sinal:]:
                            print('PADRÃO DA ESTRATÉGIA ', estrategia, ' CONFIRMADO!')
                            print('ENVIANDO SINAL TELEGRAM')
                            enviar_sinal_telegram(estrategia)
                            time.sleep(10)
...
def checar_sinal_enviado(lista_resultados_validacao, estrategia):
    global table
    ...
    while contador_cash <= int(estrategia[-1]):
        ...
        try:
            ...
            if lista_resultados_validacao != lista_resultados_sinal:
                ...
                if lista_resultados_sinal[-1] == estrategia[-2]:
                    ...
                    with open('espera.txt', 'r', encoding='UTF-8') as arquivo:
                        time.sleep(int(arquivo.read()))
...
def cadastrarEstrategia(message):
    ...
    if contador_passagem == 0:
        ...
        markup_tipo = markup_tipo.add('ESTRATÉGIAS PADRÕES', 'NOVA ESTRATÉGIA', '◀ Voltar')    

        message_opcao = bot.reply_to(message, "🤖 Ok! Deseja Usar as Estratégias Padrões ou Cadastrar uma Nova Estratégia? 👇", reply_markup=markup_tipo)
        bot.register_next_step_handler(message_opcao, registrarEstrategia)
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot Crazy Time Iniciado! ✅ Escolha uma opção 👇",
                                reply_markup=markup)
...
def registrarEstrategia(message_estrategia):
    ...
    if message_estrategia.text in ['ESTRATÉGIAS PADRÕES']:
        #Init keyboard markup
        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True, row_width=2)
        markup = markup.add('/start','✅ Ativar Bot','⚙ Cadastrar Estratégia','📜 Estratégias Cadastradas','🗑 Apagar Estratégia','♻ Resetar Resultados', '📊 Placar Atual','📈 Gestão','🛑 Pausar Bot')

        estrategias_padroes = (['ausência','1','1','1','1','1'],)
        ...
        message_aposta = bot.reply_to(message_estrategia, "🤖 Estratégias Cadastradas com Sucesso ✅", reply_markup=markup)
        ...
        resposta_usuario = message_estrategia.text.lower()
        print(resposta_usuario)
         
        markup_nova_estrategia = markup.add('◀ Voltar')

        message_nova_estrategia = bot.reply_to(message_estrategia, "🤖 Ok! Insira se deseja repetição ou ausência, Seguindo da Sequencia de Números e/ ou Bônus que Será o Robô Terá que Identificar, onde a Última Condição Será a Aposta \n\n Ex: repetição,1,2,bonus,1,2", reply_markup=markup_nova_estrategia)
        bot.register_next_step_handler(message_nova_estrategia, registrar_nova_estrategia)
...
@bot.message_handler()
def registrar_nova_estrategia(message_nova_estrategia):
    global estrategia
    global estrategias
    global placar_estrategia
    global placar_estrategias
    ...
    if message_nova_estrategia.text in ['◀ Voltar']:
        #Init keyboard markup
        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
        markup = markup.add('/start','✅ Ativar Bot','⚙ Cadastrar Estratégia','📜 Estratégias Cadastradas','🗑 Apagar Estratégia','♻ Resetar Resultados', '📊 Placar Atual','📈 Gestão','🛑 Pausar Bot')

        message_opcoes = bot.reply_to(message_nova_estrategia, "🤖 Escolha uma opção 👇",
                                reply_markup=markup)
        return

    
    estrategia = message_nova_estrategia.text
    placar_estrategia = message_nova_estrategia.text
    ...
    bot.reply_to(message_nova_estrategia, "🤖 Estratégia cadastrada com sucesso! ✅", reply_markup=markup)
