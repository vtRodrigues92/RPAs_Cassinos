...
def requisicao():
    with open("api.txt","r") as arquivo:
        url_magnify = arquivo.readlines()

        session = requests.Session()
        response = session.get(url_magnify[0], stream=True)
    ...
    if f'{data_hoje}.txt' in arquivos_placares:
        ...
        with open(f"placar/{data_hoje}.txt", 'r') as arquivo:
            try:
                ...
                placar_loss = int(arq_placar[1].split(',')[1])
                placar_geral = int(placar_win) + int(placar_loss)
                asserividade = arq_placar[2].split(',')[1]+"%"
...
def logar_site():
    ...
    #logger = logging.getLogger()
    browser.get(r"https://estrelabet.com/ptb/games/detail/casino/normal/19364")
    try:
...
def enviar_sinal(vela_atual, estrategia):
    ...
    try:
        ...
        payload = {
                ...
                'chat_id': [key for key,value in canais.items()],
                'content': mensagem_sinal[16].replace('\n','') + '\n\n' + \
        ...
        while True:
            try:
                ...
                try:
                    ...
                    for chunk in response.iter_content(chunk_size=50000):
                        ...
                        try:
                            ...
                            if '"stop":true}' in dados_api:
                                ...
                                # Salvando o ultimo resultado na variável
                                vela_atual = get_json_in_str(dados_api)['current']
                                # Add ultimo resultado na lista
                    ...
                    print(e)
                    print('ERRO NA REQUISIÇÃO --- NOVA TENTATIVA EM ALGUNS INSTANTES')
                    continue
    ...
    try:
        for estrategia in estrategias:
            ...
            if validador.count(True) == int(sequencia_minima_alerta):
                ...
                # Deixando a Lista de proximo resultado igual a Lista de resultados
                lista_proximo_resultados = lista_resultados
                ...
                #####response = requests.get(url_magnify[0], stream=True)
                # Iterando os dados vindos da API
                
                response = requisicao()

                for chunk in response.iter_content(chunk_size=50000):
                    #Decodificando os dados da API para UTF-8
                    dados_api = chunk.decode('UTF-8')

                    try:
                        # Se encontrar a palavra chave nos dados corrente, chama a função que pega a ultima vela
                        if '"stop":true}' in dados_api:
                            # Salvando o ultimo resultado na variável
                            vela_atual = get_json_in_str(dados_api)['current']
                            # Add ultimo resultado na lista
    ...
    while contador_cash <= 2:
        ...
        try:
            ...
            for chunk in response.iter_content(chunk_size=50000):
                ...
                # Verificando se deu LOSS GALE2, Sair do FOR
                if contador_cash > 2:
                    break
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    payload = {
                                            ...
                                            'chat_id': [key for key,value in canais.items()],
                                            'content': mensagem_green[32].replace('\n','').replace('[RESULTADO]', ' | '.join(resultados)),
                                            'link_refer':[value[1] for key,value in canais.items()],
                                            'link_game_bet':[value[0] for key,value in canais.items()]
                                    }
        ...
        try:
            ...
            try:
                ...
                payload = {
                        ...
                        'chat_id': [key for key,value in canais.items()],
                        'content': mensagem_green[34].replace('\n','').replace('[RESULTADO]', ' | '.join(resultados)),
                        'link_refer':[value[1] for key,value in canais.items()],
                        'link_game_bet':[value[0] for key,value in canais.items()]
                }
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot MAGNIFYMAN Iniciado! ✅ Escolha uma opção 👇",
                                reply_markup=markup)
