...
def inicio():
    ...
    url_api = 'https://blaze.com/api/crash_games/recent'
    ...
    while True:
        ...
        while True:
            try:
                ...
                historico_velas = requests.get(url_api).json()
                
                ''' Inserindo velas na lista'''
                try:

                    for crash in reversed(historico_velas[:10]):
                ...
                if estrategias == []:
                    # Se Não Tiver Estratégia Cadastrada, Validar a Estratégia do Ygor
                    validar_estrategia_ygor(lista_resultados)
                else:
        ...
        for estrategia in estrategias:
            ...
            if validador.count(True) == int(sequencia_minima_alerta):
                ...
                while True:
                    ...
                    numeros_recentes_validacao = requests.get(url_api).json()
                        
                    ''' LISTA DO PROXIMO RESULTADO APOS O ALERTA'''
                    lista_proximo_resultados = []
                    try:

                        for crash in reversed(numeros_recentes_validacao[:10]):
                            vela = crash["crash_point"]
                            if vela == '0':
                                vela = '1.00'
...
def validar_estrategia_ygor(lista_resultados):

    estrategia = ['+1','+1.5','+1','2x','2']

    print(lista_resultados)

    if float(lista_resultados[-2]) >= 1.00 and float(lista_resultados[-2]) <= 1.49 and float(lista_resultados[-1]) >= 1.50 and float(lista_resultados[-1]) <= 1.99:
        ...
        while True:
            ...
            if lista_resultados != lista_proximo_resultados:
                ...
                ''' VALIDA SE ENVIA SINAL OU APAGA O ALERTA '''
                if float(lista_proximo_resultados[-1]) >= 1.00 and float(lista_proximo_resultados[-1]) <= 1.49:
                    print(lista_proximo_resultados[-1])
                    print('ENVIA SINAL TELEGRAM')
                    print('=' * 100)
                    vela_atual = lista_proximo_resultados[-1]
...
def checar_sinal_enviado(lista_proximo_resultados, estrategia):
    ...
    # Lendo o txt config-mensagens para Pegar a qntd de Gale
    txt = open("config-mensagens.txt", "r", encoding="utf-8")
    arquivo = txt.readlines()
    gale = arquivo[37].replace('qntd_gale = ','').replace('\n','')
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot Crash 3x Iniciado! ✅ Escolha uma opção 👇",
                                reply_markup=markup)
