    ...
    def run(self):
        while True:
            ...
            try:
                ...
                #POST PARA A API
                self.post_api('confirm', mensagem=texto.split('✍️')[0].split('💰')[0] if '✍️' in texto else texto.split('💰')[0],
                                link_game=texto.split('https://')[1].split('\n')[0].replace(')','')  if '💰' in texto else '', 
                                link_refer=texto.split('https://')[2].split('\n')[0].replace(')','') if '✍️' in texto else '')
                ...
                #POST PARA A API
                self.post_api('success', mensagem=texto.split('✍️')[0] if '✍️' in texto else texto, 
                                link_game='',
                                link_refer='')
