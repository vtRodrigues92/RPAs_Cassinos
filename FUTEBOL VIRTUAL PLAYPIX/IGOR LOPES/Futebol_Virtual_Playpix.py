...
def estrategia_ambas_marcam(lista_placares):
    ...
    try:
        ...
        sequencia_minima_alerta = repeticao_ambas_marcam -1 
            ...
            while True:
                ...
                try:
                    ...
                    if lista_placares != lista_placares_validacao:
                        ...
                        if '0' in lista_placares_validacao[0]:
...
def estrategia_over_dois_e_meio(lista_placares_somados):
    ...
    try:
        ...
        sequencia_minima_alerta = repeticao_over_dois_e_meio -1
...
repeticao_ambas_marcam = ''
repeticao_over_dois_e_meio = ''
martingale = ''
...
def cadastrarEstrategia(message):
    ...
    if contador_passagem == 0:
        ...
        markup = markup.add(
                            f'Ambas Marcam = {repeticao_ambas_marcam}',
                            f'Over 2.5 = {repeticao_over_dois_e_meio}',
                            '◀ Voltar')

        escolha_config_entrada = bot.reply_to(message, "🤖 Perfeito! Escolha uma opção 👇",
                                reply_markup=markup)
            
            #Here we assign the next handler function and pass in our response from the user. 
        bot.register_next_step_handler(escolha_config_entrada, editar_estrategia)
        #bot.register_next_step_handler(message_repeticao, registrar_repeticao)
    
    else:
        #Init keyboard markup
        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True, row_width=2)
        markup = markup.add('/start','✅ Ativar Bot', '⚙ Cadastrar Repetição', '🔂 Cadastrar MartinGale', '📊 Placar Atual', '🛑 Pausar Bot')
...
@bot.message_handler(commands=['🔂 Cadastrar MartinGale'])
def martingale(message):
    ...
    if contador_passagem == 0:
        ...
        escolha_config_entrada = bot.reply_to(message, "🤖 Perfeito! Escolha um valor para o MartinGale 👇",
                                reply_markup=markup)
            
        #Here we assign the next handler function and pass in our response from the user. 
        bot.register_next_step_handler(escolha_config_entrada, registrar_martingale)
        #bot.register_next_step_handler(message_repeticao, registrar_repeticao)
...
def opcoes(message_opcoes):
    ...
    if message_opcoes.text in ['🔂 Cadastrar MartinGale']:
        print('Cadastrar MatinGale')
        martingale(message_opcoes)
...
def editar_estrategia(escolha_config_entrada):
    ...
    if escolha_config_entrada.text in ['◀ Voltar']:
        ...
        escolha_config_entrada = bot.reply_to(escolha_config_entrada, "🤖 Perfeito! Escolha uma opção 👇",
                                                reply_markup=markup)



    if escolha_config_entrada.text.split(' =')[0] in ['Ambas Marcam']:
        ...
        message_editar_estrategia = bot.reply_to(escolha_config_entrada, "🤖 Massa! Agora insira a quantidade de repetições 👇",
                                reply_markup=markup)
    ...
    if escolha_config_entrada.text.split(' =')[0] in ['Over 2.5']:
...
def editar_campo_escolhido(message_editar_estrategia):
    global repeticao_ambas_marcam, repeticao_over_dois_e_meio
    ...
    try:
        ...
        if message_editar_estrategia.text in ['◀ Voltar']:
                ...
                escolha_config_entrada = bot.reply_to(message_editar_estrategia, "🤖 Perfeito! Escolha uma opção 👇",
                                        reply_markup=markup)
            ...
            if resposta_usuario == 'Ambas Marcam':
                repeticao_ambas_marcam = int(message_editar_estrategia.text)
        

            if resposta_usuario == 'Over 2.5':
                repeticao_over_dois_e_meio = int(message_editar_estrategia.text)
            ...
            entrada_cadastrada = bot.reply_to(message_editar_estrategia, f"🤖 Repetição de {resposta_usuario} Cadastrado com Sucesso!✅",
                                        reply_markup=markup)

            bot.register_next_step_handler(entrada_cadastrada, editar_estrategia)
...
def registrar_martingale(escolha_config_entrada):
    global martingale
    ...
    try:
        ...
        if escolha_config_entrada.text in ['◀ Voltar']:
            ...
            martingale = int(escolha_config_entrada.text)

            #Init keyboard markup
            markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True, row_width=2)
            markup = markup.add('/start','✅ Ativar Bot', '⚙ Cadastrar Repetição', '🔂 Cadastrar MartinGale', '📊 Placar Atual', '🛑 Pausar Bot')

            valor_cadastrado = bot.reply_to(escolha_config_entrada, f"🤖 {str(martingale)} MartinGales Cadastrado com Sucesso!✅",
                                reply_markup=markup)
