...
def logar_site():
    ...
    #logger = logging.getLogger()
    browser.get(r"https://betfast.io/br/casino/gamepage?gameid=12489")
    try:
    ...
    try:
        ...
        while c < 10:
            try:
                browser.find_element_by_xpath('//*[@class="sound-button sound-off-btn"]').click()
                break
            except:
                c+=1
                time.sleep(3)
    ...
    while True:
        ...
        while True:
            try:
                ...
                try:
                    lista_resultados = browser.find_elements_by_xpath('//*[@class="list-wrapper"]')[0].text.split('\n')
        ...
        for estrategia in estrategias:
            ...
            if validador.count(True) == int(sequencia_minima_alerta):
                ...
                while True:
                    ...
                    lista_proximo_resultados = browser.find_elements_by_xpath('//*[@class="list-wrapper"]')[0].text.split('\n')
                    lista_proximo_resultados.reverse()
...
def checar_sinal_enviado(lista_proximo_resultados, estrategia):
    ...
    while contador_cash <= int(gale):
        ...
        try:
            ''' Lendo novos resultados para validação da estratégia'''
            lista_resultados_sinal = browser.find_elements_by_xpath('//*[@class="list-wrapper"]')[0].text.split('\n')
            lista_resultados_sinal.reverse()
        ...
        try:
            ...
            for key, value in canais.items():
                try:
                    ...
                    msg_loss = mensagem_green[39].replace('\n','').replace('[LISTA_RESULTADOS]', ' | '.join(resultados)) + '\n' +\
                                mensagem_green[40].replace('\n','').replace('[GREEN]',str(placar_win)).replace('[LOSS]', str(placar_loss))+'\n'+\
                                mensagem_green[41].replace('\n','').replace('[MAIOR_2X]', str(placar_2x))+'\n'+\
                                mensagem_green[42].replace('\n','').replace('[ASSERTIVIDADE]', str(round(placar_win / (placar_geral)*100, 0)))
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot Dino BetFast Iniciado! ✅ Escolha uma opção 👇",
                                reply_markup=markup)
