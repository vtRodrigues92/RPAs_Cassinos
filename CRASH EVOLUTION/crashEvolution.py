...
def processo_pegar_tokenapi():
    ...
    token_game = acessar_game(uuid_usuário)
...
def acessar_game(uuid_usuário):
    ...
    URL_FORNECEDOR = f'https://pi.njoybingo.com/game.do?token={uuid_usuário}&pn=meskbet&lang=pt&game=DARWIN-CRAE&type=CHARGED'
    token_fornecedor = requests.get(URL_FORNECEDOR, allow_redirects=False).headers.get('location').split('token=')[1].split('&p')[0]

    header = {
    'Content-Type': 'application/json',
    "referer": f"https://wac.paconassa.com/?game=CRAE&token={token_fornecedor}&pn=meskbet&lang=pt&type=CHARGED"
    }

    #Usando o Token gerado pelo FORNECEDOR para Acessar o GAME e pegar o TOKEN para ter acesso aos dados do GAME
    URL_GAME = f'https://wac.paconassa.com/?game=CRAE&token={token_fornecedor}&pn=meskbet&lang=pt&type=CHARGED'
    token_game = requests.get(URL_GAME,allow_redirects=False).cookies._cookies['wac.paconassa.com']['/']['sessionid'].value

    return token_game


def conexao_websocket(token_game, username, uuid_usuário):
    global URL_WS
    global ws
    global cont
    ...
    while True:
        ...
        socket_params = f'currency=BRL&token={uuid_usuário}&apiVersion=1.1.0'

        URL_WS = f'wss://wac.paconassa.com/ws/multiplayer/meskbet/CRAE/{username}/?session_token={token_game}&socket_params={socket_params}'
        
        try:
        
            ws = create_connection(URL_WS, verify=False)
...
def coletar_dados():
    ...
    global lista_base_resultados

    lista_base_resultados = ''

    conexao_websocket(token_game, username, uuid_usuário)
        ...
        while True:
            try:
                ...
                try:
                    ...
                    if 'wait' in ultimo_result:
                        try:

                            lista_resultados = json.loads(ultimo_result)[3]
                        ...
                        if lista_resultados == lista_base_resultados:
                            continue

                        print(datetime.now().strftime('%H:%M'))
                        ''' Chama a função que valida a estratégia para enviar o sinal Telegram '''
                        validar_estrategia(estrategias)   #Lista de estrategia
                        #Separador
                        print('=' * 100)
                    ...
                    print(e)
                    if e.args[0] == 'Connection to remote host was lost.':
                        time.sleep(5)
                        processo_pegar_tokenapi()
                        coletar_dados()
                    elif e.args[1] == 'Uma conexão estabelecida foi anulada pelo software no computador host':
                        conexao_websocket(token_game, username, uuid_usuário)

                    elif e.args[1] == 'Foi forçado o cancelamento de uma conexão existente pelo host remoto':
                        time.sleep(3)
                        processo_pegar_tokenapi()
                        conexao_websocket(token_game, username, uuid_usuário)
...
def validar_estrategia(estrategias):
    global gale
    global vela_atual
    global ws
    global lista_resultados
    global lista_proximos_resultados
    global lista_base_resultados
    ...
    try:
        for estrategia in estrategias:
            ...
            if validador.count(True) == int(sequencia_minima_alerta):
                ...
                vela_atual = str(lista_resultados[-1])
                ...
                while True:
                    ...
                    try:
                        ...
                        if 'wait' in ultimo_result:
                            ...
                            lista_proximos_resultados = json.loads(ultimo_result)[3]

                            if lista_proximos_resultados == lista_base_resultados:
                                continue

                            if lista_proximos_resultados != lista_resultados:

                                print(lista_proximos_resultados[-1])

                                validador = validador_estrategia(estrategia, lista_proximos_resultados, sequencia_minima_sinal)
                                ...
                                if validador.count(True) == int(sequencia_minima_sinal):
                                    ...
                                    vela_atual = str(lista_proximos_resultados[-1])
                                    lista_base_resultados = lista_proximos_resultados
                                    enviar_sinal(vela_atual, estrategia)
                                    print('=' * 100)
                                    time.sleep(5)
                                    checar_sinal_enviado(estrategia)
                                    time.sleep(5)
                                    break
                                    ...
                                    print('=' * 100)
                                    apagar_alerta()
                                    lista_resultados = lista_proximos_resultados
                                    break
...
def checar_sinal_enviado(estrategia):
    global table
    ...
    while contador_cash <= 2:
        ...
        try:
            ...
            if 'wait' in ultimo_result:
                ...
                lista_resultados_sinal = json.loads(ultimo_result)[3]

                if lista_proximos_resultados != lista_resultados_sinal:
                
                    resultados.append(str(lista_resultados_sinal[-1]))
                    ...
                    if float(lista_resultados_sinal[-1]) >= float(estrategia[-2].strip('xX')):
                        ...
                        lista_proximos_resultados = lista_resultados_sinal
                        return
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot CRASH EVOLUTION Iniciado! ✅ Escolha uma opção 👇",
                                reply_markup=markup)
