...
#def alimenta_banco_painel(lista_resultados):
#    global lista_anterior
#    '''banco de dados
#    host: sql728.main-hosting.eu
#    User: u253295982_aviator
#    Pass: Aviator22
#    DB: u253295982_dbaviator
#    Tabela: '''
#
#    campos()
#
...
def logar_site():
    ...
    while True:
        try:
            ...
            browser.get(r"https://eu-server.ssgportal.com/GameLauncher/Loader.aspx?GameCategory=JetX&GameName=JetX&ReturnUrl&Token=ce6b55f0-28a3-40ee-8f13-7148804153bd&PortalName=meskbet")  # PRODUÇÃO
            time.sleep(10)
...
def enviar_sinal(vela_atual):
    ...
    data = [
        ...
        ["🌐 <a href='https://mesk.bet/casino/?game=0'>JetX</a>     "]
    ]
    ...
    if canal_free != '':
        ...
        headers_free = ['-✅1º CASH OUT EM ' + estrategia[-2] + '\n✅2º CASH OUT EM 2x' if estrategia[-2] == '1.5x' else '-✅1º CASH OUT EM ' + estrategia[-2] + '\n✅2º CASH OUT EM 5x']

        data_free = [
            ['⏰ ENTRAR APÓS O RESULTADO '+ vela_atual                                                                  ],
            ['🔰 FAZER ATÉ ' + estrategia[-1] + ' PROTEÇÕES' if int(estrategia[-1]) > 0 else '🔰 FAZER NENHUMA PROTEÇÃO'     ],
            ["🌐 <a href='https://mesk.bet/register/meskbonus'>CRIE SUA CONTA AGORA</a>     "]
        ]
        
        table_free = columnar(data_free, headers_free, no_borders=True)
        bot.delete_message(canal_free, alerta_free.message_id)
        message_canal_free = bot.send_message(canal_free, table_free, parse_mode='HTML', disable_web_page_preview=True)
...
def checar_sinal_enviado(lista_proximo_resultados, estrategia):
            ...
            if lista_proximo_resultados != lista_resultados_sinal:
                ...
                if float(lista_resultados_sinal[-1]) >= float(estrategia[-2].strip('xX')):
                    ...
                    try:
                        ...
                        if canal_adm != '':
                            bot.edit_message_text(table +"  \n======================= \n           GREEN ✅ --- 🎯 "+ lista_resultados_sinal[-1]+"x", message_canal_adm.sender_chat.id, message_canal_adm.message_id, parse_mode='HTML', disable_web_page_preview=True)   
                        ...
                        if stop_loss.count('win') == 20:
                            try:
                                ...
                                if canal_adm !='':
                                    bot.send_video(canal_adm, video=open('money-donald-duck.mp4', 'rb'), supports_streaming=True)
    ...
    if contador_cash > int(estrategia[-1]):
        ...
        try:
            ...
            if canal_adm !='':
                bot.edit_message_text(table +"\n======================= \n              RED ✖", message_canal_adm.sender_chat.id, message_canal_adm.message_id, parse_mode='HTML', disable_web_page_preview=True)
...
CHAVE_API = '5656831413:AAHJ-2gQLbdnsVWbhp7YKcC6L2-Kt98Qawc'  # PRODUÇÃO
...
def start(message):
    ...
    if str(message.chat.id) in id_usuario:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot JetX PRO Iniciado! ✅ Escolha uma opção 👇",
                                reply_markup=markup)
