...
'pauloRamosFreitas2022@gmail.com'
...
def manipula_aba():
    ...
    try:
        ...
        browser.execute_script("window.open('https://www.vembetar.com', '_blank')")
        time.sleep(15)
...
def logarSite():
    ...
    try:
        browser.get('https://www.vembetar.com')
    ...
    #ACESSANDO TELA DO JETX
    try:

        time.sleep(3)
        browser.get('https://www.vembetar.com/casino/game/39176-jetx')
        time.sleep(10)
    ...
    #TELA CHEIA
    try:
        tela_cheia = browser.find_element_by_xpath('//*[@id="game"]/iframe').get_attribute('src')
...
def enviar_alerta_telegram(estrategia):
...
def enviar_sinal_telegram(vela_atual):
    global table_sinal
    ...
    try:
        ...
        texto = mensagem_sinal[13].replace('\n','') + '\n\n' +\
    ...
    while True:
        ...
        while True:
            try:
                ...
                #Iframe
                try:
                    iframe_game = browser.find_element_by_id('game-frame')
                    browser.switch_to_frame(iframe_game)
                except:pass
                ...
                try:
                    for vela in reversed(historico_velas[1:15]):
                        numero = vela.text
    ...
    try:
        for estrategia in estrategias:
            ...
            if validador.count(True) == int(sequencia_minima_alerta):
                ...
                enviar_alerta_telegram(estrategia)
                ...
                while True:
                    ...
                    try:
                        ...
                        try:
                            for numeroRecente in reversed(numeros_recentes_validacao[1:15]):
                        ...
                        if lista_resultados != lista_proximo_resultados:
                            ...
                            if validador.count(True) == int(sequencia_minima_sinal):
                                ...
                                enviar_sinal_telegram(vela_atual)
...
def checkSinalEnviado(lista_proximo_resultados, estrategia):
                ...
                if float(lista_resultados_sinal[-1]) >= float(estrategia[-2].strip('Xx')):
                    ...
                    try:
                        ...
                        for key, value in canais.items():
                            ...
                            try:
                                responderMensagemTelegram(key, mensagem_green[29].replace('[RESULTADO]', ' | '.join(resultados))).start()
                                time.sleep(0.1)
                            except:
                                pass
    ...
    if contador_cash > 2:
        ...
        try:
            ...
            for key, value in canais.items():
                try:
                    responderMensagemTelegram(key, mensagem_red[31].replace('\n','').replace('[LISTA_RESULTADOS]', ' | '.join(resultados))).start()
                    time.sleep(0.1)
                except:
                    pass
