...
def conexao_websocket():
    ...
    #################### DADOS PARA A LEITURA DO WEBSOCKET DO JOGO ############################
...
def inicio():
    ...
    #URL PARA O POST DA API DA FLIXBET 
            ...
            try:
                ...
                try:
                    ...
                    while True:
                        ...
                        if 'hash' in ultimo_result:

                            try:

                                vela_atual = round(float(ultimo_valor_armazenado.split(':')[-1].replace('}}"]','')),1)
    ...
    while contador_cash <= 2:
        ...
        try:
            ...
            if 'hash' in ultimo_result:
                ...
                # Add Resultado na lista de Resultados
...
#conexao_websocket()  # Conctando com o WebSocket
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot GOBLIN RUN Iniciado! ✅ Escolha uma opção 👇",
                                reply_markup=markup)
