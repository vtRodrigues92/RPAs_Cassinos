...
print()
print('                                #################################################################')
print('                                ###############  BOT LUVA SUPER GOL CRASH  ######################')
print('                                #################################################################')
...
def processo_pegar_tokenapi():
    global token_autorizacao, uuid_usuário, token_game, username
...
def acessar_game(uuid_usuário):
    ...
    URL_FORNECEDOR = f'https://pi.njoybingo.com/game.do?token={uuid_usuário}&pn=meskbet&lang=pt&game=DARWIN-LUVASG&type=CHARGED'
    token_fornecedor = requests.get(URL_FORNECEDOR, allow_redirects=False).headers.get('location').split('token=')[1].split('&p')[0]

    header = {
    'Content-Type': 'application/json',
    "referer": f"https://wac.paconassa.com/?game=LUVASG&token={token_fornecedor}&pn=meskbet&lang=pt&type=CHARGED"
    }
...
def conexao_websocket(token_game, username, dados_acesso):
    global URL_WS
    global ws
    global cont
    ...
    while True:
        ...
        socket_params = f'currency=BRL&token={dados_acesso}&apiVersion=1.1.0'

        URL_WS = f'wss://wac.paconassa.com/ws/multiplayer/meskbet/LUVASG/{username}/?session_token={token_game}&socket_params={socket_params}'
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot LUVA SUPER GOL Iniciado! ✅ Escolha uma opção 👇",
                                reply_markup=markup)
...
def opcoes(message_opcoes):
    ...
    if message_opcoes.text in ['✅ Ativar Bot']:
        ...
        if botStatus == 1:
            ...
            #Função que Pega o Link do WS gerado
            processo_pegar_tokenapi()
            coletar_dados()
