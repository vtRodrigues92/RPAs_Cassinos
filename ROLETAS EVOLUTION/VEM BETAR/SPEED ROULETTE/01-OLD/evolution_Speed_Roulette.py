...
def auto_refresh():
    ...
    tres_hora = timedelta(minutes=10)
...
# Validar Jogo Pausado
def valida_jogo_pausado():

    try:

        sessao_expirada = browser.find_elements_by_class_name('titleContainer--fe91d')
        for se in sessao_expirada:
            if se.text == 'SESSÃO EXPIRADA':
                logarSite()
...
def logarSite():
    browser.get(r"https://vembetar.com/ptb/bet/main")
    try:
...
def coletarResultados(lista_roletas):
    ...
    while True:
        ...
        browser.get(site_cassino_desktop)
                ...
                while True:
                    ...
                    try:
                        ...
                        if lista_resultados[:int(sequencia_minima)] != lista_proximo_resultados[:int(sequencia_minima)]:
                            ...
                            if lista_proximo_resultados[0] in aposta_externa[estrategia[2]] or estrategia[2] == '0':
                                ...
                                ultimo_resultado = lista_proximo_resultados[0]
                                enviarSinalTelegram(nome_cassino, sequencia_minima, estrategia, nome_fornecedor, ultimo_resultado)
                                print('=' * 150)
                                checkSinalEnviado(lista_proximo_resultados, nome_cassino, ultimos_resultados)
                                time.sleep(1)
                                break
...
def checkSinalEnviado(lista_proximo_resultados, nome_cassino, ultimos_resultados):
    global table_sinal
    global placar_win
    global placar_semGale
    global placar_gale1
    ...
    while contador_cash <= 2:
        ...
        try:
            ...
            if lista_proximo_resultados[:int(sequencia_minima)] != lista_resultados_sinal[:int(sequencia_minima)]:
                ...
                if lista_resultados_sinal[0] in grupo_apostar[estrategia[4]] or lista_resultados_sinal[0] == '0' or lista_resultados_sinal[0] == '00':
