...
print()
print('                                #################################################################')
print('                                ########### BOT ROLETAS PRAGMATIC & EVOLUTION ###################')
print('                                #################################################################')
...
def buscar_aposta(estrategia_usuario):
    global opcoes_apostas

    with open ('arquivos_txt//aposta.txt', 'r', encoding='UTF-8') as arquivo:
        apostas = arquivo.read()

    opcoes_apostas = ast.literal_eval(apostas)
...
def formatar_resultados_pragmatic(roleta):  
    ...
    try:
        ...
        for numero in reversed(resultados):
            if 'x' not in numero:
...
def logarSite():
    ...
    while True:
        try:
            ...
            #time.sleep(2)
            browser.find_element_by_name('email').send_keys('victor.o.rodrigues11@gmail.com') 
            time.sleep(2)
            #Inserindo senha
            browser.find_element_by_name('current-password').send_keys('Fordbracom2022')
            time.sleep(2) 
...
def coletarResultados(lista_roletas):
    ...
    global horario_atual
    global nome_cassino, link_roleta, nome_fornecedor
    
    #fornecedor = {'pragmatic':['https://www.arbety.com/games/external/0cd292ec4e38445c8976579594d5d1d4','//*[@id="ROULETTE"]/*[name()="li"]'], 
    #              'evolution':['https://www.arbety.com/games/external/5564f22a260c4e8daa807eed6e68d065','//*[@class="GridListItem--b95c7"]']}
    evolution = 0

    fornecedor = {'evolution':['https://www.arbety.com/games/external/5564f22a260c4e8daa807eed6e68d065','//*[@class="GridListItem--b95c7"]']}
            ...
            if key == 'evolution':
                ...
                while c < 10:
                    try:
                        ...
                        browser.get('https://tmkybox.evo-games.com/frontend/evo/r2/#category=roulette&game=roulette')
            ...
            try:
                ...
                for roleta in roletas:
                    ...
                    try:
                        ...
                        nome_fornecedor = key
                        nome_cassino = roleta.text.split('\n')[2] if key == 'pragmatic' else roleta.text.split('\n')[-3]
                        link_roleta = value[0]
...
def validarEstrategia(dicionario_roletas, nome_cassino, lista_estrategias, roleta):
    global estrategia
    global contador_passagem
    global lista_resultados_sinal, grupos_estrategia, lista_proximo_resultados
    ...
    try:
        ...
        for estrategia in lista_estrategias:
            ...
            grupos_estrategia = estrategia.split(';')
            sequencia_minima_alerta = len(estrategia.split(';')[:-2])
            sequencia_minima_sinal = len(estrategia.split(';')[:-1])
            
            print ('Analisando a Estrategia --> ', estrategia)
            print('Historico_Roleta --> ', nome_fornecedor, ' | ', nome_cassino, dicionario_roletas[nome_cassino])

            ''' Verifica se os números da seq minima do historico da roleta está dentro da estratégia '''
            validador = validarEstrategiaAlerta(dicionario_roletas, nome_cassino, sequencia_minima_alerta, estrategia)
            
            print(validador)

            ### LENDO ARQUIVO DE VALIDADOR DE SINAL PRA VER SE OUTRO BOT ESTRA ENVIANDO SINAL
            with open('arquivos_txt/validador_sinal.txt', 'r') as arquivo:
                validador_sinal = arquivo.read()
            ...
            if validador.count('true') == int(sequencia_minima_alerta):
                ...
                print('ENVIAR ALERTA')
                enviarAlertaTelegram(dicionario_roletas, nome_cassino, sequencia_minima_alerta, link_roleta)
                time.sleep(1)
                ...
                while True:
                    try:
                        ...
                        if dicionario_roletas[nome_cassino][:3] != lista_proximo_resultados[:3]:
                            ...
                            print('Historico_Roleta --> ', nome_cassino, lista_proximo_resultados[:int(sequencia_minima_sinal)])

                            ''' Verificando se o ultimo resultado da roleta está dentro da estratégia'''
                            if int(lista_proximo_resultados[-1]) in ast.literal_eval(grupos_estrategia[-2]):

                                dicionario_roletas[nome_cassino] = lista_proximo_resultados
                                print('ENVIANDO SINAL TELEGRAM')
                                enviarSinalTelegram(nome_cassino, sequencia_minima_sinal, link_roleta)
                                print('=' * 220)
...
def validarEstrategiaAlerta(dicionario_roletas, nome_cassino, sequencia_minima_alerta, estrategia):
    validador = []
    c_resultdo = -sequencia_minima_alerta
    c_grupo_estrategia = 0

    while c_grupo_estrategia < sequencia_minima_alerta:
        for n_grupo in range(len(grupos_estrategia[:-sequencia_minima_alerta])):

            if int(dicionario_roletas[nome_cassino][c_resultdo]) in ast.literal_eval(grupos_estrategia[c_grupo_estrategia]):
                validador.append('true')
                c_resultdo +=1
                c_grupo_estrategia +=1
            else:
...
def enviarAlertaTelegram(dicionario_roletas, nome_cassino, sequencia_minima, link_roleta):
    global contador_passagem
...
def enviarSinalTelegram(nome_cassino, sequencia_minima_sinal, link_roleta):
    global table_sinal
            ...
            try:
                ...
                globals()[f'sinal_{key}'] = bot.send_message(key, 
                                                               message_sinal.replace('[NOME_FORNECEDOR]', nome_fornecedor.title())
                                                                          .replace('[APOSTA]', grupos_estrategia[-1].title())
                                                                          .replace('[NOME_CASSINO]', nome_cassino.title())
                                                                          .replace('[LISTA_RESULTADOS]', lista_proximo_resultados[-1])
                                                                          .replace('[SITE_DESKTOP]', link_roleta).replace('[SITE_MOBILE]', link_roleta)
                                                                          .replace('[SITE_CADASTRO]', value[0]), 
                                                               parse_mode='HTML', disable_web_page_preview=True)
            except:
    ...
    while contador_cash <= int(gale):
        ...
        try:
            ...
            if lista_proximo_resultados[:3] != lista_resultados_sinal[:3]:
                ...
                grupo_apostar = buscar_aposta(grupos_estrategia[-1])

                # VALIDANDO WIN OU LOSS
                if lista_resultados_sinal[-1] in grupo_apostar or lista_resultados_sinal[-1] == '0' or lista_resultados_sinal[-1] == '00':
...
def cadastrarEstrategia(message):
    ...
    if contador_passagem == 0:
        ...
        message_estrategia = bot.reply_to(message, "🤖 Ok! Insira o Grupo de Números que Deverá Sair nas Rodadas. \n *** INSIRA OS NÚMEROS ENTRE '' Ex: ['1','2','3'];['4','5','6'];['7','8','9'] 👇", reply_markup=markup_tipo)
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, f"🤖 Olá {message.json['from']['first_name']}, voce está usando o Bot Inteligente de Roletas Pragmatic & Evolution! Escolha uma opção 👇",
                                    reply_markup=markup)
...
def registra_estrategia(message_estrategia):
    ...
    global lista_estrategias, txt_estrategias_vazio, resposta_usuario
    ...
    if message_estrategia.text in ['◀ Voltar']:
        ...
        message_opcoes = bot.reply_to(message_estrategia, "🤖 Escolha uma opção 👇", reply_markup=markup)
        ...
        dict_apostas = ast.literal_eval(apostas)

        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
        markup_apostas = generate_buttons_estrategias([key for key, value in dict_apostas.items()], markup)    
        markup_apostas.add('◀ Voltar')

        resposta_usuario = message_estrategia.text
        
        #lista_estrategias.append(message_usuario)
        print(resposta_usuario)

        ond_apostar = bot.reply_to(message_estrategia, "🤖 Perfeito! Agora, quando o sinal for enviado para o Canal, onde os jogadores irão apostar?", reply_markup=markup_apostas)
...
def registraOndeApostar(ond_apostar):
    ...
    if ond_apostar.text in ['◀ Voltar']:
        #Init keyboard markup
        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True, row_width=2)
        markup = markup.add('/start','✅ Ativar Bot','⚙🧠 Cadastrar Estratégia', '⚙🎰 Cadastrar Roletas', '🧠📜 Estratégias Cadastradas', '🎰📜 Roletas Cadastradas', '🗑🧠 Apagar Estratégia', '🗑🎰 Apagar Roleta', '⏲ Ultimos Resultados', '📊 Placar Atual','📈 Gestão','🛑 Pausar Bot')

        message_opcoes = bot.reply_to(ond_apostar, "🤖 Escolha uma opção 👇", reply_markup=markup)
        return
        ...
        resposta_usuario2 = ond_apostar.text.lower()
        onde_apostar = resposta_usuario + ';' + resposta_usuario2
