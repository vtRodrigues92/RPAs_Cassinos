...
def conectar_websocket(evosessionid):
    ...
    URL=f'wss://aleaplay.evo-games.com/public/topcard/player/game/TopCard000000003/socket?messageFormat=json&instance=oi1xyz-rrdjwbcrr5uagjd7-TopCard000000003&tableConfig=&EVOSESSIONID={evosessionid}&client_version=6.20231206.71959.35599-15033a1fb1'                                                                                                                                                                                                            
    ...
    try:
        for key,value in canais.items():
            try:
                ...
                ''' Lendo o arquivo txt config-mensagens '''
                txt_alerta = open("alerta.txt", "r", encoding="utf-8").read()

                ''' Mensagem '''
                mensagem_alerta = txt_alerta\
                                    .replace('[SITE_PC]', value[0])\
                                    .replace('[SITE_MB]', value[0])\
                                    .replace('[LINK_CADASTRO]', value[1])
                

                globals()[f'alerta_{key}'] = enviarAlertaTelegram(key, mensagem_alerta).start()
                time.sleep(0.2)
    ...
    try:
        for key, value in canais.items():
            try:
                ...
                ''' Lendo o arquivo txt config-mensagens '''
                txt_alerta = open("sinal.txt", "r", encoding="utf-8").read()

                ''' Mensagem '''
                mensagem_sinal = txt_alerta\
                                    .replace('[SITE_PC]', value[0])\
                                    .replace('[SITE_MB]', value[0])\
                                    .replace('[LINK_CADASTRO]', value[1])\
                ...
                globals()[f'sinal_{key}'] = enviarSinalTelegram(key, mensagem_sinal).start()
                    ...
                    try:
                        ...
                        for key, value in canais.items():
                            try:
                                ...
                                ''' Lendo o arquivo txt green'''
                                txt_green = open("green.txt", "r", encoding="utf-8").read()

                                responderMensagemTelegram(key, txt_green.replace('[RESULTADO]', ' | '.join(resultado_valida_sinal))).start()
                                time.sleep(0.1)
        ...
        try:
            ...
            for key,value in canais.items():
                try:
                    ...
                    ''' Lendo o arquivo txt green'''
                    txt_red = open("red.txt", "r", encoding="utf-8").read()

                    responderMensagemTelegram(key, txt_red).start()
                    time.sleep(0.1)
