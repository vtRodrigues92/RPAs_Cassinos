...
def enviarSinalTelegram(nome_cassino, url_cassino, sequencia_minima, estrategia):
    ...
    table_sinal = mensagem_sinal[20].replace('\n','') + '\n\n' +\
...
def checkSinalEnviado(lista_proximo_resultados, nome_cassino, roleta):
            ...
            if lista_proximo_resultados[:3] != lista_resultados_sinal[:3]:
                ...
                if lista_resultados_sinal[0] in grupo_apostar[estrategia[3]] or lista_resultados_sinal[0] == '0' or lista_resultados_sinal[0] == '00':
                    ...
                    try:
                        ...
                        for canal in canais:
                            try:
                                bot.reply_to(globals()[f'sinal_{canal}'], mensagem_green[41].replace('\n','').replace('[RESULTADO]', ' | '.join(resultados))\
                                             .replace('[PULA_LINHA]','\n'), parse_mode='HTML')
                            except:
                                pass
    ...
    if contador_cash == 3:
        ...
        try:
            ...
            for canal in canais:
                try:
                    bot.reply_to(globals()[f'sinal_{canal}'], mensagem_green[43].replace('\n','').replace('[LISTA_RESULTADOS]', ' | '.join(resultados)), parse_mode = 'HTML')
                except:
                    pass
