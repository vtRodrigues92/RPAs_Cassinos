...
def alimenta_banco_appData(lista_resultados):
    ...
    '''banco de dados
    IP: 185.239.210.5
    User: u791277084_useraviator
    Pass: 7prGB3!LYD#
    DB: u791277084_dbaviator
    Tabela: app_data (histórico dos jogos), app_game (resultados dos sinais)'''

    if lista_anterior == [] or lista_anterior != lista_resultados:

        vela_atual = lista_resultados[-1].replace(',','')

        try:
            ''' CONECTANDO COM O BANCO '''
            db_conexao = mysql.connector.connect(host='191.96.63.106', database='u791277084_dbaviator', user='u791277084_useraviator', password='7prGB3!LYD#')

            #Variavel que executa as querys
            cursor = db_conexao.cursor()

            ''' QUERY '''
            query_inserir_dados = (f"""INSERT INTO u791277084_dbaviator.app_data 
...
def alimenta_banco_appGame(lista_resultados, status):
    ...
    vela_atual = lista_resultados_sinal[-1].replace(',','')
...
def logar_site():
    ...
    #logger = logging.getLogger()
    browser.get(r"https://estrelabet.com/en/games/detail/casino/normal/7787")
...
def checar_sinal_enviado(lista_proximo_resultados, estrategia):
    ...
    if contador_cash > int(estrategia[-1]):
        ...
        try:
            ...
            for key, value in canais.items():
                try:
                    bot.reply_to(globals()[f'sinal_{key}'], mensagem_green[26].replace('\n','').replace('[LISTA_RESULTADOS]', ' | '.join(resultados)), parse_mode = 'HTML')
                except:
                    pass
