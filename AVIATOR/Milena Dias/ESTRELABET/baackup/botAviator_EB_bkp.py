...
class enviarPostAPI(threading.Thread):
    def __init__(self, canais, status, mensagem, estrategia):
    ...
    def run(self):
        ...
        payload = {
                ...
                'content':  self.mensagem[0].replace('\n','') + '\n\n' + \
                            self.mensagem[2].replace('\n','') + '\n\n' + \
                            self.mensagem[4].replace('\n','').replace('[OPERADOR]', 'maior' if self.estrategia[-3][0] == '+' else 'menor').replace('[ULTIMA_CONDICAO]', self.estrategia[-3][1:]),
...
# THREAD PARA ENVIAR MENSAGEM TELEGRAM
class enviarMensagemTelegram(threading.Thread):
...
def logar_site():
    ...
    try:
        ...
        ''' Acessando o jogo '''
        browser.get('https://estrelabet.com/en/games/detail/casino/normal/7787')
        # MODO TELA CHEIA
        contador = 0
        while contador<10:
            try:

                tela_cheia = browser.find_element_by_id('gm-frm').get_attribute('src')
...
def enviar_alerta(estrategia):
    ...
    try:
        ...
        enviarPostAPI(canais, 'alert', mensagem_alerta, estrategia).start()

    except Exception as e:
        print(e)
    
    # Enviando Mensagem Telegram
    inincio = datetime.now()
    ...
    try:
        for key, value in canais.items():
            try:
                ...
                enviarMensagemTelegram(key, table_alerta).start()
    ...
    print('MENSAGEM ENVIADA PARA TODOS OS CANAIS EM ---> ', datetime.now() - inicio)
