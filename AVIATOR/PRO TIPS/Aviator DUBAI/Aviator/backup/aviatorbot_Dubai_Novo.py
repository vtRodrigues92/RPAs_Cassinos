    ...
    while True:
        ...
        try:
            ...
            if flew_away[0] == 'FLEW AWAY!':
                ...
                if contador == 0 and len(contagem_vela) <= 8:
                    for estrategia in estrategias:
                        for e in estrategia:
                            ...
                            for v in contagem_vela:
                                ...
                                if e[0] == '-':
                                    resultado = operator.lt(float(v[:-1]), float(e[1:]))
                                    validacao.append(resultado)
                ...
                if contador == 1 and contagem_vela == []:
                    try:
    ...
    while contador <= int(estrategia[-1]):
        ...
        try:
            ...
            if flew_away[0] == 'FLEW AWAY!':
                ...
                if float(vela) > float(estrategia[-2].replace('x','')):
...
def estrategiasCadastradas(message):
    ...
    for estrategia in estrategias:
        print(estrategia[:-4])
        bot.send_message(message.chat.id, f'{estrategia[:-4]}')
...
@bot.message_handler(commands=['📈 Gestão']) #### CONSTRUIINDO
def gestao(message):
    global placar
    global resultados_sinais
...
def opcoes(message_opcoes):
    ...
    if message_opcoes.text in ['✅ Ativar Bot']:
        ...
        #Init keyboard markup
        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
        markup = markup.add('/start', '🆓 Enviar sinais Canal FREE', '🏆 Enviar sinais Canal VIP', '🆓🏆 Enviar sinais Canal FREE & VIP')
...
def registrarEstrategiaExcluida(message_excluir_estrategia):
    ...
    for estrategia in placar_estrategia:
        if estrategia_excluir == str(estrategia[:-4]):
            placar_estrategia.remove(estrategia)
