...
def formata_dados(ultimo_result):
    ...
    resultado = json.loads(ultimo_result)['args']['game']['result']  

    if resultado == 'HomeWon':
        resultado = 'C'
        
    if resultado == 'AwayWon':
        resultado = 'V'
        
    if resultado == 'Draw2to11':
        resultado = 'E'
...
def start(message):
    ...
    if str(message.chat.id) in ids:
        ...
        message_opcoes = bot.reply_to(message, "🤖 Bot Football Studio DICE Iniciado! ✅ Escolha uma opção 👇",
                                reply_markup=markup)
