...
def pegar_evosessionid():
    ...
    payloadEvo = '{"requestBody":{"gameId":"18198","channel":"web","vendorLimitId":"evol_TopCard000000001_BRL","vendorId":130,"redirectUrl":"https://vembetar.com/ptb/games/livecasino/detail/normal/18198/evol_TopCard000000001_BRL"},"identity":null,"device":"d","languageId":23}'
    headersEvo = {"Content-Type": "application/json","Origin":"https://vembetar.com","s7oryO9STV":s7oryo9stv,"X-PGDevice":"m"}
...
def conectar_websocket(evosessionid):
    ...
    try:
        ...
        "Sec-WebSocket-Key": "+J/7DNPHfftdL0GuinfJpw==",
        ...
        URL = f'wss://nesvembataross.evo-games.com/public/topcard/player/game/TopCard000000001/socket?messageFormat=json&instance=2raq0w-reruxm6epoju7jjb-TopCard000000001&tableConfig=&EVOSESSIONID={evosessionid}&client_version=6.20230703.70738.27351-85ddb7dd11'
...
def logar_site():
    ...
    browser.get(r'https://pi.njoybingo.com/game.do?token=dd7a4fe2-f1d8-4b55-a8fd-274219a399cc&pn=meskbet&lang=pt&game=EVOLUTION-topcard-nvrpqglt6teqkvaf&type=CHARGED')
                ...
                try:
                    ...
                    if 'resolved' in ultimo_result:
                        ...
                        if estrategia[:sequencia_minima_sinal] == lista_resultados[-sequencia_minima_sinal:]:
                            ...
                            enviar_sinal_telegram(lista_resultados)
                            time.sleep(7)
...
def enviar_alerta_telegram():
    ...
    ''' Lendo o arquivo txt '''
    with open('arquivos_txt/msg_alerta.txt',"r", encoding="utf-8") as arquivo:
...
def enviar_sinal_telegram(lista_resultados):
    global table_sinal
    ...
    ''' Lendo o arquivo txt config-mensagens '''
    with open('arquivos_txt/msg_sinal.txt',"r", encoding="utf-8") as arquivo:
        ...
        for key, value in canais.items():
            try:
                ...
                globals()[f'sinal_{key}'] = bot.send_message(key, message_sinal
                                                             .replace('[COR]', '🔴' if estrategia[-1] == 'C' else '🔵' if estrategia[-1] == 'V' else '⚪' )
                                                             .replace('[RESULTADO_ATUAL]', '🔴' if lista_resultados[-1] == 'C' else '🔵' if lista_resultados[-1] == 'V' else '⚪')
                    ...
                    try:
                        ...
                        for key, value in canais.items():
                            try:
                                ...
                                bot.reply_to(globals()[f'sinal_{key}'], message_green.replace('[RESULTADO]', resultado), parse_mode='HTML')
