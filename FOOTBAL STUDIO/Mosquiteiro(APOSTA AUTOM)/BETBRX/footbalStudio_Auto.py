...
def validarJogoPausado():
    try:
        ...
        for sessao in terminou_sessao:
            if sessao.text == 'The session ended. Close this window and log in again to play.' \
            or sessao.text == 'You have been disconnected. Close this window and connect again to play.' \
            or sessao.text == 'This table is temporarily inactive. Come back later or select a different table.':
                logarSite()


        sessao_expirada = browser.find_elements_by_css_selector('.titleContainer--fe91d')
        for sessao in sessao_expirada:
            if sessao.text == 'SESSION EXPIRED' or sessao.text == 'There was a problem. Please try again later or choose a different table.':
                logarSite()


        att_pagina = browser.find_elements_by_css_selector('.content--c7c5e')
        for pagina in att_pagina:
            if pagina.text == 'Reload the page to return to the game on this device.' \
            or sessao.text == 'You have been disconnected. Close this window and connect again to play.' \
            or sessao.text == 'This table is temporarily inactive. Come back later or select a different table.':
                logarSite()
...
def logarSite():
    ...
    while True:
        ...
        try:
            browser.get(r"https://www.betbrx.com/pt-br/play/998/topcard--TopCard000000003?demo=false")
            ...
            try:
                browser.find_element_by_id('email').send_keys(usuario)   #Inserindo login
                browser.find_element_by_id('password').send_keys(senha)  #Inserindo senha
                browser.find_element_by_xpath('//*[@class="wlc-btn wlc-btn--size-default wlc-btn--theme-mod-secondary wlc-btn--theme-rounding wlc-btn--type-default ng-star-inserted"]').click()    #Clicando no btn login
                ...
                t3 = 0
                while t3 < 20:
                    if browser.find_elements_by_xpath('//*[@class="value"]'):
                        break
                    else:
                        t3+=1
            except:
            ...
            browser.get(browser.find_element_by_xpath('//*[@class="egamings_game_frame"]').get_attribute('src'))
    ...
    try:
        if int(protecao) != 0:
            ...
            table_sinal = mensagem_sinal[9].replace('\n','') + '\n' +\
                          mensagem_sinal[10].replace('\n','').replace('[VALOR_APOSTA]', 'R$'+str(valor_entrada)).replace('[COR]','🟨' if estrategia[-1] == 'H' else '🟦' if estrategia[-1] == 'A' else '⬜') + '\n' +\
                          mensagem_sinal[11].replace('\n','').replace('[VALOR_PROTEÇÃO]','R$'+str(protecao))
        
        else:
            table_sinal = mensagem_sinal[9].replace('\n','') + '\n' +\
                          mensagem_sinal[10].replace('\n','').replace('[VALOR_APOSTA]', 'R$'+str(valor_entrada)).replace('[COR]','VERMELHO' if estrategia[-1] == 'H' else 'AZUL' if estrategia[-1] == 'A' else 'AMARELO')
    ...
    while contador_cash <= martingale:
        ...
        try:
            ...
            if lista_resultados_validacao != lista_resultados_sinal:
                ...
                if lista_resultados_sinal[-1] == estrategia[-1] or lista_resultados_sinal[-1] == 'D' and protecao != 0:
