...
def validarJogoPausado():
    try:
        ...
        for pagina in att_pagina:
            if pagina.text == 'Atualize a página para voltar a entrar no jogo neste dispositivo':
                logarSite()
    ...
    try:
        if browser.find_element_by_xpath('//*[@id="root"]/div/div[2]/div/div/div[6]/div[1]/span').text == 'JOGO EM PAUSA DEVIDO A INATIVIDADE':
            browser.find_element_by_xpath('//*[@id="root"]/div/div[2]/div').click()
    
    except:
        try:
            if browser.find_element_by_xpath('//*[@id="root"]/div/div[2]/div/div/div[10]/div[1]/span').text == 'JOGO EM PAUSA DEVIDO A INATIVIDADE':
                browser.find_element_by_xpath('//*[@id="root"]/div/div[2]/div').click()
            ...
            try:
                ...
                try:
                    if browser.find_element_by_xpath('//*[@id="root"]/div/div[2]/div/div[2]/div[5]/div/div/div/div[1]/div[2]').text == 'O seu saldo é demasiado baixo para se juntar a esta mesa.':
                        logarSite()
                except:
                    try:
                        if browser.find_element_by_css_selector('#loader-popup label').text == 'Something went wrong during loading the game. Please try again later or select a different table in the Lobby.':
                            logarSite()
                    except:
                        pass
...
def checkSinalEnviado(lista_resultados_validacao):
        ...
        try:
            ...
            if lista_resultados_validacao != lista_resultados_sinal:
                ...
                if lista_resultados_sinal[-1] == estrategia[-1] or lista_resultados_sinal[-1] == 'E':
                    ...
                    if stop_win.count('win') == 3:
                        print('STOP WIN -- VOLTAREI EM 30 MIN')
                        time.sleep(1800)
                        stop_win = []
    ...
    if contador_cash == 3:
        ...
        if stop_loss.count('loss') == 1:
            try:
                print('LOSS - VOLTAREI EM 30 MIN')
                time.sleep(1800)
            except:
                pass
        ...
        if stop_loss.count('loss') == 2:
            try:
                ...
                if canal_free !='':
                    bot.send_message(canal_free, f'⛔🛑 Atenção!!,\nOs padrões mudaram e nesse momento, o melhor a se fazer é STOPARo robô!\n\nVoltaremos quando o mentor estiver presente para verificar os sinais.\n\nAtt, Diretoria Pro Tips 🤝 ')

                if canal_vip !='':
                    bot.send_message(canal_vip, f'⛔🛑 Atenção!!,\nOs padrões mudaram e nesse momento, o melhor a se fazer é STOPARo robô!\n\nVoltaremos quando o mentor estiver presente para verificar os sinais.\n\nAtt, Diretoria Pro Tips 🤝 ')
                
                if canal_adm !='':
                    bot.send_message(canal_adm, f'⛔🛑 Atenção!!,\nOs padrões mudaram e nesse momento, o melhor a se fazer é STOPARo robô!\n\nVoltaremos quando o mentor estiver presente para verificar os sinais.\n\nAtt, Diretoria Pro Tips 🤝 ')

                print('STOP LOSS - VOLTAREI EM 30 MIN')
                stop_loss = []
